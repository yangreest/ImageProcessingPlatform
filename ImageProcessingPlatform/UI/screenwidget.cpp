...
void ScreenWidget::closeEvent(QCloseEvent *)
{ 
...
void ScreenWidget::mouseMoveEvent(QMouseEvent *e)
{
    if (screen->getStatus() == Screen::SELECT) {
        ...
        QPoint p(e->position().x() - movPos.x(), e->position().y() - movPos.y());
        screen->move(p);
        movPos = e->pos();
        // 设置按钮位置在截图区域下方或右方
        QPoint buttonPos = screen->getRightDown();
        releaseButton->move(QPoint(buttonPos.x()- releaseButton->width(),buttonPos.y()));
    }
...
void ScreenWidget::mouseReleaseEvent(QMouseEvent *)
{
...

    {
        ...
        // 可以连接按钮点击事件
        connect(releaseButton, &QPushButton::clicked, this, &ScreenWidget::onButtonClicked);
    }
}
...
void ScreenWidget::onButtonClicked()
{
    // 处理按钮点击事件，比如保存截图、关闭窗口等
    emit signalSaveScreen(this->getSelectPixmap());
    this->close();
}
